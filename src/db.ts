/**
 * SQLite database access layer for the Belgian Financial Regulation MCP server.
 *
 * Schema mirrors the FSMA/NBB regulatory structure:
 *   - sourcebooks        — top-level regulatory sourcebook directories (FSMA_Circulaires, NBB_Circulaires, etc.)
 *   - provisions         — individual circulaires, communications, guidance, and directions
 *   - enforcement_actions — FSMA and NBB enforcement decisions
 *
 * FTS5 virtual tables back full-text search on provisions and enforcement actions.
 * Primary language: French (matching FSMA/NBB publication language).
 */
...
export function getProvision(
  ...
  // Fuzzy fallback: match by reference prefix
  return (
    (db
      .prepare(
        "SELECT * FROM provisions WHERE sourcebook_id = ? AND reference LIKE ? LIMIT 1",
      )
      .get(sourcebook.toUpperCase(), `${reference}%`) as Provision | undefined) ??
    null
  );
